...
public:
    ...
    virtual void processRefund(const string& transactionId, double amount) = 0;
    virtual string getGatewayName() = 0;
    virtual ~IPaymentGateway() {}
};

class SberGateway : public IPaymentGateway {
public:
    ...
    void processRefund(const string& transactionId, double amount) override {
...
class TbankGateway : public IPaymentGateway {
public:
    void processPayment(const string& paymentDetails, double amount) override {
...
class AlfaGateway : public IPaymentGateway {
public:
    void processPayment(const string& paymentDetails, double amount) override {
...
public:
    PaymentMethod(IPaymentGateway* gw) : gateway(gw) {}
...
public:
    CreditCardPayment(IPaymentGateway* gw, string number, string holder,
    ...
    void refund(const string& transactionId, double amount) override {
        ...
        gateway->processRefund(transactionId, amount);
    }
...
public:
    ContactlessPayment(IPaymentGateway* gw, string app, string device, string authToken)
...
public:
    SBPPayment(IPaymentGateway* gw, string phone, string bank)
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 4) {
        ...
        IPaymentGateway* gateway = nullptr;
        ...
        if (method == "card") {
            ...
            string holder = (argc > 5) ? argv[5] : "Ivanov I.";
