...
class SudokuSnapshot:
    """Хранитель: неподдающийся изменению снимок состояния"""
...
class SudokuGame:
    ...
    def set_cell(self, r: int, c: int, val: int) -> bool:
        ...
        #если нарушены правила судоку, то счётчик ошибок увеличивается
...
class SaveManager:
    ...
    def undo(self) -> Optional[SudokuSnapshot]:
        if len(self.undo_stack) > 1:  #остается минимум 1 снимок (начальное состояние)
...
class SudokuApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.manager.push(self.game.create_memento())  #сохранение начального состояния
    ...
    def _on_cell_input(self, r: int, c: int, *args):
        ...
        # Применение паттерна: сохранение снимка после изменения
        self.manager.push(self.game.create_memento())
        self._update_status()

    def _update_grid_ui(self):
        """Синхронизация View с Model (после Undo/Redo)."""
